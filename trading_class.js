...
class API {
    constructor(address,private_key,chainId) {
        ...
        this.transaction_url = (chainId != 1) ? `https://${this.chain_name}.etherscan.io/tx/` : `https://etherscan.io/tx/`
    ...
    async testTrade() {
        ...
        const tx = contract.methods.tokenToEthTransferOutput(tokensSold,minEth,deadline,addressFrom);
    ...
    async swapFromEth(outToken,amountIn) {
        ...
        const trade = new Trade(route, new TokenAmount(WETH[outToken.chainId], amountIn), TradeType.EXACT_INPUT)
        ...
        const tx = routerContract.methods.swapExactETHForTokens(web3.utils.toHex(String(amountOutMin)),path,addressFrom,deadline)
        const encodedABI = tx.encodeABI()
    ...
    async swapToEth(inToken,amountIn) {
        ...
        const amountOutMin = trade.minimumAmountOut(slippageTolerance).raw // needs to be converted to e.g. hex
        console.log('amountOutMin',String(amountOutMin),web3.utils.fromWei(String(amountOutMin),'ether'))
        ...
        let gasPrices = await this.getCurrentGasPrices()
        const addressFrom = this.address
        const approvalTx = web3.eth.abi.encodeFunctionCall(APPROVE_JSON,[process.env.UNISWAP_ROUTER_ADDRESS,web3.utils.toHex(amountIn)])
    ...
    async swapToToken(inToken,outToken,amountIn) {
        ...
        const deadline = Math.floor(Date.now() / 1000) + 15 // 15 seconds
        const path = [route.input.address,route.output.address]
        const tx = routerContract.methods.swapExactTokensForTokens(web3.utils.toHex(amountOutMin),path,addressFrom,deadline)
        const encodedABI = tx.encodeABI()
        let gasPrices = await this.getCurrentGasPrices()
        web3.eth.getTransactionCount(addressFrom).then(txCount => {
            // construct the transaction data
    ...
    async checkLiquidity(tokenName) {
        ...
        // let pairAddress = this.getPairAddress(token.address)
        const pair = await Fetcher.fetchPairData(token, this.weth, provider);
        // console.log('pair',pair)
        // console.log('pairAddress',pairAddress)
        // let myBalanceWei = await web3.eth.getBalance(pairAddress)
        // const exchangeContract = new web3.eth.Contract(JSON.parse(factory_abi.Exchange_abi), process.env.UNISWAP_FACTORY_ADDRESS,
        //     (error,result) => {if (error) {console.log(error)}})
        // let exchange = exchangeContract.methods.getExchange(pairAddress)
        // // console.log(exchange)
        // console.log(exchange._parent._address)
        const tokenLiquidity = JSBI.divide(pair.tokenAmounts[0].numerator, pair.tokenAmounts[0].denominator).toString()
        const wethLiquidity = JSBI.divide(pair.tokenAmounts[1].numerator, pair.tokenAmounts[1].denominator).toString()
        console.log(tokenLiquidity / wethLiquidity)
        console.log(wethLiquidity / tokenLiquidity)
        console.log(tokenLiquidity)
        console.log(wethLiquidity)
        // let myBalanceWei = await web3.eth.getBalance(pair.liquidityToken.address)
        // console.log('checkLiquidity',myBalanceWei)
        // console.log(Object.getOwnPropertyNames(exchangeContract.methods))
    }
    ...
    tokenLookup(tokenName) {
        ...
        if (typeof token !== 'undefined') {
            return new Token(this.chainId, token.tokenAddress, token.decimals,tokenName)
        } else {
            throw 'No such token'
        }
    }
    ...
    getPairAddress(tokenAddress) {
        let toEth = getCreate2Address(FACTORY_ADDRESS,keccak256(['bytes'], [pack(['address', 'address'], [tokenAddress,this.weth.address])]),INIT_CODE_HASH)
        return toEth
    }
